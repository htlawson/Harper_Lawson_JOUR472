...
md_tracts <- read_xls("maryland_tracts.xls")
md_counties <- read.table("md_counties_geoids.txt", header = TRUE, sep = "\t")
...
md_smith_data %>%
  ...
  group_by(NAME) %>%
  summarise(average = mean(census_response_rate2020, na.rm = TRUE)) %>%
  arrange(desc(average))

# Howard County has the highest census response rate for 2020 and Dorchester County has the lowest.
...
# The resulting table will have missing values. For example, being that "Kent County, Maryland" and just "Kent County" represent the same location but are phrased differently, these two columns would not match during the join. In order to ensure that the data in both columns (and others) matches up, you would either need to correct the column names or use the [table1 %>% inner_join(table2, by=c("table1_column"="table2_column"))] code.
