...
1.  One question one could answer with a spatial join is what political party the majority of the populations of given areas follow. One could answer this question using an SF data frame if they were really just looking for numbers, but a leaflet data frame could be used to generate an interactive visualization of this answer.
2.  Another question one could answer with a spatial join is the wealthiest of given areas. A ggplot2 or leaflet data frame could be used to visualize wealthier populations in a given area by using population demographics and data on household incomes.
