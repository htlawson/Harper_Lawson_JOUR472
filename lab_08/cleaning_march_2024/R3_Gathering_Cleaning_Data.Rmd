...
Pull data into R through various means, including accessing APIs and web
URLs Clean and structure the data for analysis
...
<img src="images/API_drivers.png" width="300" height="150"/> <br>\
<br>

-- \*\*Click on green arrow icon and that will display path directory in
the console in R Studio Copy that path into your code
...
Feeds to homepage map <https://www.datawrapper.de/_/HZNsT/> This is
autoloaded to Github via the vaccine_county.csv

Feeds to vaccine chart <https://www.datawrapper.de/_/dDKzc/>
...
```{r}
copspeak <- rio::import("Radio_Codes_2016 (3).xlsx", which ="RadioCodes")
head(copspeak)
...
-   **Serious Actions**: Create a table with the serious actions
    including citations and arrests police took in the dispositions
...
Find out when nothing happened with the officer's investigation. A table
filtering the dispositions column to show "no disposition" or "gone on
arrival"
...
For this lesson, we will clean just one column. See Appendix on the
method to clean and construct a new dataframe with all of the columns
...
Build a summary table with the days of the week with the greatest number
of calls. Create a graphic. Then build a table to see if the complaints
vary by day
