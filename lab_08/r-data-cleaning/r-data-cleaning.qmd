...
**The method I would use to clean this data would be to use the mutate() and case_when() functions. Firstly, there is a discrepancy in the state names. Maryland, MD and md all mean the same thing, but case_when() would be necessary to ensure this is recognized. This also applies to the county names. Having a mix of abbreviated and spelled-out county names can cause issues, so case_when() could also be used again to replace abbreviations with full names or vice versa. While the population values are formatted the same, we could encounter the error seen earlier in this document of the values being "alphabetized." To prevent this, we could use mutate() to set every population value to numeric.**
